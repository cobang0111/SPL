...
from config.train_llm_preference_model import (
    DataSubset,
    get_hh_rlhf_dataset,
...
def generate_embeddings_with_llm(args, input_dataset=None):
    ...
    preprocessed_dataset = input_dataset.map(
        HHRLHFPreprocessor(tokenizer, **tokenizer_kwargs),
        batched=True,
        num_proc=24,
    ...
    print(f"Length of input dataset = {len(input_dataset)}")
    print(f"Length of preprocessed dataset = {len(preprocessed_dataset)}")
    ...
    for row_id in tqdm(range(dataset_size)):
        ...
        for key in ['chosen', 'rejected']:
            ...
            with torch.no_grad():
                ...
                vec = last_hidden_state[0][token_length - 1].float().cpu().tolist()
