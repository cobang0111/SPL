...
class IVPLModel(nn.Module):
    ...
    def reparameterization(self, mean, std, deterministic=False, x=None):
        # base sample
        if deterministic:
        ...
        z0 = mean + std * eps  # z_0
        log_q0 = Normal(mean, std).log_prob(z0).sum(-1)
        ...
        for iaf in self.iaf_flows:
            conditioned_iaf = iaf.condition(x)
            zk_next = conditioned_iaf(zk)
            ld = conditioned_iaf.log_abs_det_jacobian(zk, zk_next)
            log_det = log_det + ld
            zk = zk_next
    ...
    def forward(
        ...
        if ground_truth_user_vector:
            ...
            z, log_qz = self.reparameterization(mean, torch.exp(0.5 * log_var), x=x)

        if not self.training and not ground_truth_user_vector:
            z, log_qz = self.reparameterization(mean, torch.exp(0.5 * log_var), deterministic=True, x=x) 
            
        rc, rr = self.decode(target_chosen, target_rejected, z)

        if self.use_iaf:
            return rc, rr, mean, log_var, z, log_qz
        else:
...
class IVPLTrainer(Trainer):
    ...
    def compute_loss(self, wrapped_model, inputs, return_outputs=False):
        if isinstance(wrapped_model, IVPLModel):
        ...
        if model.use_iaf:
            rewards_chosen, rewards_rejected, mean, log_var, z, log_qz = model(
                embeddings_chosen,
                embeddings_rejected,
        ...
        if self.kl_loss_weight == 0:
            ...
            if model.use_iaf:
                ...
                with torch.no_grad():
                    rc_sw, rr_sw, mean_sw, log_var_sw, z_sw, log_qz_sw = model(
                            embeddings_rejected, 
                            embeddings_chosen,
                            contexts_embeddings_rejected, 
                            contexts_embeddings_chosen,
